...
function HourlyForecast({ hourly, loading }: { hourly: HourlyPoint[]; loading: boolean }) {
  const [open, setOpen] = useState(false);
  // Filter to current hour and forward, then take next 24
  const now = Date.now();
...
function HourlyGraph({ hours }: { hours: HourlyPoint[] }) {
  const [hoverIdx, setHoverIdx] = useState<number | null>(null);
  const W = 100; // viewBox width units (per-hour spacing)
  const H = 100;
  ...
  const tempY = (t: number) => 10 + (1 - (t - tMin) / tRange) * 55; // top half for temp
